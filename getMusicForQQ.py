...
for n in range(0, len(mids)):
    ...
    for m in range(0, x):
        ...
        try:
            #urllib.request.urlretrieve(srcs[m], 'd:/music/QQ/' + songnames[m] + ' - ' + singers[m] + '.m4a')
            urllib.request.urlretrieve(srcs[m], 'D:/MyResources/Multi-Media-File/Music/QQ/' + songnames[m] + ' - ' + singers[m] + '.m4a')
        except:
            x = x - 1
